...
class PVClient(fl.client.NumPyClient):
    ...
    def __init__(self, parque):
        self.model = PVModel(input_size=len(FEATURES) * (len(LAGS) + 1), layers_sizes=[128, 32, 16])
        ...
        # ORDEN TEMPORAL
        # =========================
        df_parque["Time Stamp (local standard time) yyyy-mm-ddThh:mm:ss"] = pd.to_datetime(
            df_parque["Time Stamp (local standard time) yyyy-mm-ddThh:mm:ss"]
        )
        df_parque = df_parque.sort_values("Time Stamp (local standard time) yyyy-mm-ddThh:mm:ss")
        df_parque = df_parque.reset_index(drop=True)
        ...
        # =========================
        df_parque["hour"] = df_parque["Time Stamp (local standard time) yyyy-mm-ddThh:mm:ss"].dt.hour
        df_parque["sin_hour"] = np.sin(2 * np.pi * df["hour"] / 24)
        df_parque["cos_hour"] = np.cos(2 * np.pi * df["hour"] / 24)
        ...
        # =========================
        for df_parque in [train_df, val_df,test_df]:
            ...
            df_parque["T_diff"] = df_parque["PV module back surface temperature (degC)"] - 25

            df_parque["poa_ghi_ratio"] = df_parque["POA irradiance CMP22 pyranometer (W/m2)"] / (
                df_parque["Global horizontal irradiance (W/m2)"] + 1e-6
            )

            df_parque["dni_ghi_ratio"] = df_parque["Direct normal irradiance (W/m2)"] / (
                df_parque["Global horizontal irradiance (W/m2)"] + 1e-6
            )

            df_parque["dhi_ghi_ratio"] = df_parque["Diffuse horizontal irradiance (W/m2)"] / (
                df_parque["Global horizontal irradiance (W/m2)"] + 1e-6
            )

            df_parque["cloud_index"] = df_parque["Diffuse horizontal irradiance (W/m2)"] / (
                df_parque["Direct normal irradiance (W/m2)"] + 1e-6
            )

            df_parque["temp_diff_air"] = (
                df_parque["PV module back surface temperature (degC)"] -
                df_parque["Dry bulb temperature (degC)"]
            )

            df_parque["poa_temp"] = df_parque["POA irradiance CMP22 pyranometer (W/m2)"] * \
                             df_parque["PV module back surface temperature (degC)"]

            df_parque["ghi_temp"] = df_parque["Global horizontal irradiance (W/m2)"] * \
                             df_parque["Dry bulb temperature (degC)"]
        ...
        for df_parque in [train_df, val_df, test_df]:
            for lag in LAGS:
                for col in FEATURES:
                    df_parque[f"{col}_lag{lag}"] = df_parque[col].shift(lag)
