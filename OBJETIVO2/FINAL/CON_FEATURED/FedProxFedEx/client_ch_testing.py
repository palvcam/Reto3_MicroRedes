...
def estimate_k_panel(train_df):
    """Estima el coeficiente k_panel basado en regresión lineal para cada panel."""
    k_por_panel = {}
    ...
    return k_por_panel, k_global

def apply_feature_engineering(df, k_por_panel, k_global):
    """Aplica las transformaciones físicas y ratios a un dataframe."""
    df = df.copy() # Buenas prácticas: no modificar el original
...
# 1. Separación de responsabilidades: La clase PVClient ya no necesita saber "cómo"
# se calcula la física de la nube o la temperatura. Solo sabe que necesita llamar a esas funciones.

#2. NOS FACILITA EL TESTEO: El testing atacará directamente a apply_feature_engineering
# sin tener que instanciar toda la red neuronal ni cargar ningún CSV.
class PVClient(fl.client.NumPyClient):
    def __init__(self, parque):
        ...
        # 2. SPLIT (Se mantiene igual)
        train_val_df, test_df = train_test_split(df_parque, test_size=0.2, random_state=42)
        train_df, val_df = train_test_split(train_val_df, test_size=0.2, random_state=42)

        # 3. ESTIMACIÓN DE k_panel (¡AQUÍ USAMOS LA FUNCIÓN!)
        # Solo calculamos con train_df para evitar data leakage
        k_por_panel, k_global = estimate_k_panel(train_df)

        # 4. FEATURES DE INGENIERÍA (¡AQUÍ USAMOS LA FUNCIÓN!)
        # Aplicamos las transformaciones físicas a cada subset
        train_df = apply_feature_engineering(train_df, k_por_panel, k_global)
        val_df   = apply_feature_engineering(val_df, k_por_panel, k_global)
        test_df  = apply_feature_engineering(test_df, k_por_panel, k_global)

        # 5. INPUT FINAL (Se mantiene igual)
        ...
        # 7. DATALOADERS (Se mantienen igual)
