...
import flwr as fl
import numpy as np  # ← NUEVO para FedEx
...
class PVClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        print(f"  [FedEx] Epochs={epochs}, MU={mu:.3f}")
        ...
        # 4. EVALUAR modelo ENTRENADO (¡NO global!)
        self.model.eval()
        mse = 0
        with torch.no_grad():
        ...
        self.epoch_probs = np.exp(-self.fedex_lr * grad * self.epoch_probs)
        self.epoch_probs /= self.epoch_probs.sum()
        self.mu_probs = np.exp(-self.fedex_lr * grad * self.mu_probs)
        self.mu_probs /= self.mu_probs.sum()
        
        # 6. DEVOLVER modelo ENTRENADO
